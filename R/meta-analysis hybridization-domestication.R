...
# subset data
hyb_dom_data <- subset(data, `hyb-dom`==1)

# calculate effect sizes
ES_data <- data.frame(ID=NULL, short_citation=NULL, crop=NULL, ES=NULL, PSD=NULL, CA_judgement=NULL)
for (i in 1:length(unique(hyb_dom_data$ID))){
  subset_data <- subset(hyb_dom_data, ID==unique(hyb_dom_data$ID)[i])
  # prepare data
...
ES_data$comparison <- rep("Hybridization - Domestication", nrow(ES_data))
